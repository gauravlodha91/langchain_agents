...
#             encoding_name="gpt2"
#         )
#         docs = token_splitter.split_text_to_documents(sample_text)
#         for i, doc in enumerate(docs, 1):
#             print(f"Document {i}:")
#             print(f"  Content: {doc.page_content[:100]}...")
#             print(f"  Metadata: {doc.metadata}")
...
#                 separator=" "
#             )
#             docs = spacy_splitter.split_text_to_documents(sample_text)
#             for i, doc in enumerate(docs, 1):
#                 print(f"Document {i}:")
#                 print(f"  Content: {doc.page_content[:100]}...")
#                 print(f"  Metadata: {doc.metadata}")
...
#     # Example 8: Document creation and splitting (CONSISTENT OUTPUT)
...
#     # Example 9: Splitting existing documents (CONSISTENT OUTPUT)
...
#     # Example 10: Using split_text_to_documents for CONSISTENT OUTPUT
#     print("\n10. USING SPLIT_TEXT_TO_DOCUMENTS FOR CONSISTENT OUTPUT:")
#     consistent_docs = recursive_splitter.split_text_to_documents(
#         sample_text, 
#         metadata={"source": "consistent_example.md", "type": "educational"}
#     )
#     print(f"Created {len(consistent_docs)} documents with consistent format")
#     for i, doc in enumerate(consistent_docs[:2], 1):  # Show first 2
#         print(f"Consistent Document {i}:")
#         print(f"  Content: {doc.page_content[:100]}...")
#         print(f"  Metadata: {doc.metadata}")
    
#     print("\n" + "=" * 60)
#     print("CONSISTENCY CHECK - All outputs are List[Document]:")
#     print("✓ CharacterTextSplitter.split_text_to_documents() -> List[Document]")
#     print("✓ RecursiveCharacterTextSplitter.split_text_to_documents() -> List[Document]") 
#     print("✓ TokenTextSplitter.split_text_to_documents() -> List[Document]")
#     print("✓ SpacyTextSplitter.split_text_to_documents() -> List[Document]")
#     print("✓ MarkdownHeaderTextSplitter.split_text() -> List[Document]")
#     print("✓ HTMLHeaderTextSplitter.split_text() -> List[Document]")
#     print("✓ PDFPlumberTextSplitter.split_pdf() -> List[Document]")
#     print("✓ PDFPlumberTextSplitter.split_text_to_documents() -> List[Document]")
#     print("✓ SentenceTransformersTokenTextSplitter.split_text_to_documents() -> List[Document]")
    
#     print("\n" + "=" * 60)
#     print("REUSABILITY FEATURES:")
#     print("✓ All Document objects have consistent structure:")
#     print("  - page_content: str (the actual text content)")
#     print("  - metadata: Dict[str, Any] (with standard fields)")
#     print("✓ Standard metadata fields across all splitters:")
#     print("  - chunk_index: int (position in sequence)")
#     print("  - chunk_id: str (unique identifier)")
#     print("  - total_chunks: int (total number of chunks)")
#     print("  - start_index: int (optional, if add_start_index=True)")
#     print("✓ Custom metadata preserved and merged with standard fields")
#     print("✓ All splitters support metadata parameter for consistency")
    
#     print("\n" + "=" * 60)
#     print("USAGE PATTERNS FOR REUSABILITY:")
#     print("1. For consistent Document output from any splitter:")
#     print("   docs = splitter.split_text_to_documents(text, metadata={'source': 'file.txt'})")
#     print("2. For LangChain compatibility:")
#     print("   docs = splitter.create_documents([text1, text2], [meta1, meta2])")
#     print("3. For splitting existing documents:")
#     print("   split_docs = splitter.split_documents(existing_docs)")
#     print("4. All outputs can be used interchangeably in downstream processing!")
...
class TextSplitter(ABC):
    ...
    def split_text_to_documents(self, text: str, metadata: Optional[Dict[str, Any]] = None) -> List[Document]:
        """Split text and return Document objects with consistent format."""
        ...
        for i, chunk in enumerate(chunks):
            ...
            if self._add_start_index:
                ...
                doc_metadata["start_index"] = start_index if start_index != -1 else 0
...
class MarkdownHeaderTextSplitter:
    ...
    def split_text(self, text: str, metadata: Optional[Dict[str, Any]] = None) -> List[Document]:
        """Split markdown file and return Document objects."""
        lines = text.split("\n")
        lines_with_metadata: List[dict] = []
        current_content: str = ""
        current_metadata: Dict[str, str] = metadata.copy() if metadata else {}
        header_stack: List[Tuple[str, str]] = []
        initial_metadata: Dict[str, str] = current_metadata.copy()
        start_index = 0
            ...
            for sep, name in self.headers_to_split_on:
                if stripped_line.startswith(sep) and (
                    ...
                    if current_content.strip():
                        doc_metadata = current_metadata.copy()
                        doc_metadata.update({
                            "chunk_index": len(lines_with_metadata),
                            "chunk_id": f"chunk_{len(lines_with_metadata)}",
                        })
                        if self.add_start_index:
                            doc_metadata["start_index"] = start_index
                        
                        lines_with_metadata.append(
                            {"content": current_content, "metadata": doc_metadata}
                        )
                        start_index += len(current_content)
                        current_content = ""
        ...
        if current_content.strip():
            ...
            doc_metadata.update({
                "chunk_index": len(lines_with_metadata),
                "chunk_id": f"chunk_{len(lines_with_metadata)}",
                "total_chunks": len(lines_with_metadata) + 1
            })
            if self.add_start_index:
                doc_metadata["start_index"] = start_index
        ...
        for item in lines_with_metadata:
            item["metadata"]["total_chunks"] = len(lines_with_metadata)
...
class HTMLHeaderTextSplitter:
    ...
    def split_text(self, text: str, metadata: Optional[Dict[str, Any]] = None) -> List[Document]:
        """Split HTML text and return Document objects."""
        ...
        current_headers: Dict[str, str] = metadata.copy() if metadata else {}
        start_index = 0
        ...
        for element in elements:
            if element.name in self.headers_to_split_on:
                ...
                if current_chunk_content.strip():
                    doc_metadata = current_headers.copy()
                    doc_metadata.update({
                    ...
                    start_index += len(current_chunk_content)
                    current_chunk_content = ""
...
class PDFPlumberTextSplitter(TextSplitter):
    ...
    def split_text(self, text: str) -> List[str]:
        ...
        raise NotImplementedError("Use split_pdf method for PDF files or use split_text_to_documents for text input.")
    
    def split_text_to_documents(self, text: str, metadata: Optional[Dict[str, Any]] = None) -> List[Document]:
        """Split text and return Document objects."""
        # For PDF splitter, we'll treat text as if it's from a single page
        base_metadata = metadata or {}
        base_metadata.update({
            "page": 1,
            "source": base_metadata.get("source", "text_input")
        })
        ...
        if self.split_by == "page":
            ...
            return super().split_text_to_documents(text, metadata)

    def split_pdf(self, pdf_path: str, metadata: Optional[Dict[str, Any]] = None) -> List[Document]:
        """Split PDF file into Document objects."""
        import fitz
        ...
        for document in documents:
            document.metadata["total_chunks"] = len(documents)
...
# UTILITY CLASS FOR UNIFIED API
class UnifiedTextSplitter:
    """
    Unified interface for all text splitters with consistent output format.
    This class ensures all splitting methods return List[Document] for maximum reusability.
    """
    
    def __init__(self):
        self.available_splitters = {
    ...
    def split_text(
        ...
        **kwargs
        ...
        """
        Universal text splitting method with consistent Document output.
        ...
        Returns:
            List[Document]: Consistent format across all methods
        """
        if method not in self.available_splitters:
            raise ValueError(f"Unknown method '{method}'. Available: {list(self.available_splitters.keys())}")
        
        splitter_class = self.available_splitters[method]
        ...
        if method == 'markdown':
            ...
            splitter = splitter_class(headers_to_split_on=headers_to_split_on, **kwargs)
            ...
            # PDF splitter needs special handling for text input
            splitter = splitter_class(**kwargs)
            return splitter.split_text_to_documents(text, metadata=metadata)
    ...
    def get_available_methods(self) -> List[str]:
        ...
        return list(self.available_splitters.keys())
    
    def get_method_info(self, method: str) -> Dict[str, Any]:
        """Get information about a specific splitting method."""
        if method not in self.available_splitters:
        ...
        return {
            'name': method,
            'class': splitter_class.__name__,
            'description': splitter_class.__doc__.strip() if splitter_class.__doc__ else "No description available",
            'required_dependencies': self._get_dependencies(method)
        }
    
    def _get_dependencies(self, method: str) -> List[str]:
        """Get required dependencies for a method."""
        deps = {
            'token': ['tiktoken'],
            'spacy': ['spacy'],
        ...
        return deps.get(method, [])


# Example usage of unified interface
if __name__ == "__main__":
    print("\n" + "=" * 60)
    print("UNIFIED TEXT SPLITTER DEMONSTRATION")
    print("=" * 60)
    
    # Create unified splitter instance
    unified = UnifiedTextSplitter()
    
    print("Available methods:", unified.get_available_methods())
    
    # Test with different methods - all return List[Document]
    sample_text = "This is a test. This is another sentence. Here's a third one."
    
    methods_to_test = ['character', 'recursive', 'token']
    
    for method in methods_to_test:
        try:
            print(f"\n{method.upper()} METHOD:")
            docs = unified.split_text(
                sample_text, 
                method=method,
                chunk_size=50,
                chunk_overlap=10,
                metadata={'source': f'{method}_test.txt', 'method': method}
            )
            
            print(f"Generated {len(docs)} documents")
            for i, doc in enumerate(docs):
                print(f"  Doc {i}: {doc.page_content[:30]}... | Metadata: {doc.metadata}")
                
        except (ImportError, ValueError) as e:
            print(f"  Skipped {method}: {e}")
    
    print(f"\n{'='*60}")
    print("✅ SUCCESS: All methods return consistent List[Document] format!")
    print("✅ Perfect for reusable code - same interface, same output structure!")
    print("✅ All chunks have standardized metadata with custom fields preserved!")
    print(f"{'='*60}")